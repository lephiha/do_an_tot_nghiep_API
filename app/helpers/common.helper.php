...
function isBirthdayValid($date)
{
    ...
    // 3.3 - Case 2 - year(2025) > yearToday(2024)
    if( $yearDifference < 0)
    {
        $output = "Today is ".$today." so that this birthday is not valid !";
        return $output;
    }
    //3.3 - Case 3 - year == yearToday
...
function isAppointmentDateValid($appointment_date)
{
    ...
    //2 - Case 1 - the date does not exist
    ...
    //2 - Case 2 - year(2025) > yearToday(2024)
    if( $yearDifference > 0 )
    {
        //always correct
    }
    // 2 - Case 3 - year(2025) == yearToday(2025) 
    else if( $yearDifference == 0)
    {
    ...
    // 2 - Case 4 - year(2019) < yearToday (2022) 
    else
    {
        $output = "Today is ".$today." so that is not valid !";
        return $output;
 ...
 * if message is not empty => it it not valid
 * 
 * For instance, today is 18-10-2024 16h00 then
 * 19-10-2024 or any date far from today is always valid to create appointments with any hour
 * 17-10-2024 is invalid to create appointments. Of course
 * 18-10-2024 10h00 is invalid
 * 18-10-2024 17h00 is valid
 */
function isAppointmentHourValid($appointment_hour, $appointment_date)
{
    ...
    if( $hour < $currentHour )
    {
        ...
        {
            /**(current)18-10-2024 17:15pm VS (appointment)18-10-2024 16:00pm & $hour must in working hours
             * => CORRECT  */
            if( $dayDifference == 0 && $currentHour < $hour && $currentHour > 7 && $currentHour < 20)
            {
                # always correct
            }
            /**(current)18-10-2024 17:15pm VS (appointment)19-10-2024 16:00pm => CORRECT  */
